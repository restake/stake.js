...
import { Algorithm, verify as verifyAlgorithm } from "./algorithm.js";
import {
    decoder,
    encoder,
    isArray,
...
function createSigningInput(header: Header, payload: Payload): string {
    return `${b64encode(encoder.encode(JSON.stringify(header)), true)}.${
        b64encode(encoder.encode(JSON.stringify(payload)), true)
    }`;
}
